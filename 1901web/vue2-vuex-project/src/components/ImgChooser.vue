    ...
    chooseFile(fid, data) {
      if (fid && data) {
        this.$emit('file-choosed', {
          fid: this.$getFileUrl(fid),
          file: data,
        })
      } else {
        this.$emit('file-choosed', null)
      }
    },
      ...
      this.$upload(
        ...
        function (data) {
          ...
          if (data.success) {
            ...
            app.fileinfo = ''
            // 获取服务端返回的文件信息
            let fileinfo = data.resultData.tbFile
            app.chooseFile(fileinfo.fid, fileinfo)
          }
          this.$notification.open({
            message: data.message,
          })
